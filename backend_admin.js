...
var registerClubEdits = function() {
    $("tbody tr").each(function() {
        $(this).children("td").eq(5).children("a").eq(0).click(function() { //Approve Edit
    ...
    $("tbody tr").each(function() {
        $(this).children("td").eq(5).children("a").eq(1).click(function() { //Delete Edit
            var id = $(this).parent().data("index");
            $.ajax({
                type: "GET",
                url: "scripts/club_query.php",
                data: "action=noapproveclubedit" + "&value=" + id,
            }).done(function() {
                window.location = "backend_admin.php";
            });
        });
